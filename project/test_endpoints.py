...
def test_avg_elapsed_time():
    ...
    data = r.json()
    assert r.status_code == 200
    assert isclose(data['Average elapsed time, sec'], 35.60, rel_tol=1e-02)
...
def test_avg_rows_per_sec():
    ...
    data = r.json()
    assert r.status_code == 200
    assert isclose(data['Average rows per sec'], 5467092.94, rel_tol=1e-02)
...
def test_avg_rows_per_thread():
    ...
    data = r.json()
    assert r.status_code == 200
    assert isclose(data['Average rows per thread'], 24856407.93, rel_tol=1e-02)
...
def test_avg_thread_per_sec():
    ...
    data = r.json()
    assert r.status_code == 200
    assert isclose(data['Average threads per sec'], 0.034, rel_tol=1e-02)