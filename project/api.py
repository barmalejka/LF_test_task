...
def avg_elapsed_time():
    ...
    if day_data.shape[0] == 0:
        return handle_wrong_data('Date is out of range')
...
def avg_rows_per_sec():
    ...
    data = pd.read_csv(os.path.join(os.path.dirname(__file__), "data/queries.tsv"), sep='\t')

    time_range_data = data[data['time'].between(datetime_start_code, datetime_end_code)]
    if time_range_data.empty:
        return handle_wrong_data(400)

    time_range_pivot = time_range_data.pivot(index='query_id', columns='status', values=['time', 'rows'])
    time_range_pivot = pd.DataFrame(time_range_pivot.to_records())
    time_range_pivot.drop(columns="('rows', 0)", inplace=True)
    time_range_pivot.columns = ['query_id', 'start', 'end', 'rows']
    time_range_pivot = time_range_pivot[~time_range_pivot.isnull().any(1)]  # takes only queries closed within period
    # milliseconds to seconds
    time_range_pivot['start'] = (time_range_pivot['start']/1000).astype('int')
    time_range_pivot['end'] = (time_range_pivot['end']/1000).astype('int')

    time_range_pivot['elapsed_time'] = (time_range_pivot['end'] - time_range_pivot['start'])
    time_range_pivot['total_rows'] = time_range_pivot['elapsed_time'] * time_range_pivot['rows']

    # finding overlapping periods
    df_start = time_range_pivot[['start']]
    df_end = time_range_pivot[['end']]
    df_start['direction'] = 1
    df_end['direction'] = -1

    df_start.columns = ('TS', 'direction')
    df_end.columns = ('TS', 'direction')

    df_start_end = pd.concat([df_start, df_end]).sort_values('TS')
    df_start_end['queries'] = df_start_end['direction'].cumsum()

    df_start_end['empty'] = (df_start_end['queries'] == 0).shift().fillna(0).astype(int)
    df_start_end['busy_id'] = df_start_end['empty'].cumsum()

    busy_data = df_start_end.groupby('busy_id')['TS'].agg([min, max])
    total_elapsed_time = (busy_data['max'] - busy_data['min']).sum()

    avg_rows = time_range_pivot['total_rows'].sum() / total_elapsed_time
...
def avg_rows_per_thread():
    ...
    time_range_data['n_threads'] = time_range_data.threads.str.split(',').str.len()
